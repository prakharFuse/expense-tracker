...
test('formatMoney formats zero cents', () => {
...
test('formatMoney formats dollars and cents', () => {
  assert.equal(formatMoney(1234), '$12.34');
});

test('formatMoney groups the integer part into thousands', () => {
  assert.equal(formatMoney(100000), '$1,000.00');
});

test('formatMoney groups the integer part into millions', () => {
...
test('formatMoney groups negative amounts while preserving the sign', () => {
  assert.equal(formatMoney(-100000), '-$1,000.00');
});
